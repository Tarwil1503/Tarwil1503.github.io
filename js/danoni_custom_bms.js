...
function customLoadingInit() {
	if (g_stateObj.gauge === 'HARD') {
		g_workObj.lifeDmg /= 10;
		g_workObj.lifeRcv /= 2;
	}
	if (['EASY','NORMAL'].includes(g_stateObj.gauge)) {
		g_workObj.lifeDmg /= 2;
		g_workObj.lifeRcv /= 2;
	}
}
...
let g_gaugelist = ['EASY', 'NORMAL', 'HARD'];
...
function customJudgeIi(difFrame){
	if (g_gaugelist.includes(g_stateObj.gauge)) {
		lifeRecovery();
	}
}
